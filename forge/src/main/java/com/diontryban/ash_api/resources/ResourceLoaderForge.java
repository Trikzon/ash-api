...
@ApiStatus.Internal
public final class ResourceLoaderForge extends ResourceLoader {
    private static final List<PreparableReloadListener> SERVER_RELOAD_LISTENERS = new ArrayList<>();

    private final PackType type;

    /**
     * Default constructor only to be used by ImplementationLoader.
     *
     * @deprecated Use {@link ResourceLoader#get(PackType)}
     */
    public ResourceLoaderForge() {
        type = null;
    }

    private ResourceLoaderForge(PackType type) {
        this.type = type;
    }
    ...
    protected @NotNull ResourceLoader getImpl(PackType type) {
        return new ResourceLoaderForge(type);
    }
}
